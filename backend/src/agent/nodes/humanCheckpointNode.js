// graph.mjs
// LangGraph orchestration – original career modes + new chat mode
...
// ------------------------------------------------------------
// STATE DEFINITION (original + chat fields)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// ORIGINAL HELPER FUNCTIONS (keep all)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// CONDITIONAL ROUTING (extended for chat mode)
// ------------------------------------------------------------
function routeFromMode(state) {
  if (state.mode === "analyze") return "diagnoseProfile";
  if (state.mode === "plan") return "retrieveRAG";
  if (state.mode === "chat") return "chatNode";   // NEW
  return "scoreCareers";
}

function afterChat(state) {
  return state.profileComplete ? "searchNode" : "chatNode";
}

function afterHumanCheckpoint(state) {
  if (state.humanApprovalNeeded) return "humanCheckpoint";
  if (state.userApproval === "rejected") return END;
  return END;
}

// ------------------------------------------------------------
// BUILD GRAPH (original nodes + new chat nodes)
// ------------------------------------------------------------
const graph = new StateGraph(State)
  ...
  // Chat flow
  .addConditionalEdges("chatNode", afterChat, ["chatNode", "searchNode"])
  .addEdge("searchNode", "buildPlan")          // reuse original plan node
...
// ------------------------------------------------------------
// EXPORTED RUN FUNCTION (extended for chat)
// ------------------------------------------------------------
